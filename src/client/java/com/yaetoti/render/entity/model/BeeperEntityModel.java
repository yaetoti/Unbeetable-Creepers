package com.yaetoti.render.entity.model;
...
public class BeeperEntityModel<T extends BeeperEntity> extends AnimalModel<T> {
    ...
    public BeeperEntityModel(ModelPart root) {
    ...
    public void animateModel(T beeEntity, float f, float g, float h) {
        ...
        // this.bodyPitch = beeEntity.getBodyPitch(h); TODO implement
    }
    ...
    public void setAngles(T beeEntity, float f, float g, float h, float i, float j) {
        ...
        boolean bl2 = bl = beeEntity.isOnGround() && beeEntity.getVelocity().lengthSquared() < 1.0E-7;
        if (bl) {
            ...
            this.rightWing.roll = MathHelper.cos((float)k) * (float)Math.PI * 0.15f;
        ...
        //if (!beeEntity.hasAngerTime()) { TODO check
            this.bone.pitch = 0.0f;
            this.bone.yaw = 0.0f;
            this.bone.roll = 0.0f;
            if (!bl) {
                k = MathHelper.cos((float)(h * 0.18f));
                ...
                this.bone.pivotY = 19.0f - MathHelper.cos((float)(h * 0.18f)) * 0.9f;
            }
        //}
